...
def single_run(config: dict):
    ...
    network = QNetworkPixel(action_dim=action_dim, n_atoms=n_atoms) if config.get("PIXEL_BASED", True) else QNetworkOC(action_dim=action_dim, n_atoms=n_atoms)
    ...
    def full_c51_step(agent_state, buffer_state, env_state, obs, rng, global_step):
        ...
        def do_update(update_carry, _):
            ...
            next_vals = (next_pmfs * atoms[None, None, :]).sum(-1)
            ...
            def q_loss_fn(params):
                ...
                pmf_pred = pmfs[jnp.arange(batch_size), b_act.reshape(-1)]
                pmf_clipped = jnp.clip(pmf_pred, 1e-5, 1 - 1e-5)
                loss = -(target_pmfs * jnp.log(pmf_clipped)).sum(-1).mean()
                q_val = (pmf_pred * atoms[None, :]).sum(-1).mean()
                return loss, q_val
    ...
    def save_and_eval(step_count):
        ...
        for mods_cfg, mod_label in eval_configs:
            episodic_returns, env_states = evaluate(
                ...
                config["ENV_ID"],
                eval_episodes=10,
                Model=QNetworkPixel if config["PIXEL_BASED"] else QNetworkOC,
                n_atoms=n_atoms,
                v_min=v_min,
                v_max=v_max,
